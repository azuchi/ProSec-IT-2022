...
#
# SOMENAME defaults to azuchi/bitcoin:0.21 in both the build.sh and
# run.sh cases.
#
...
mkdir -p $datadirmp
docker run -p 38333:38333 -v $datadirmp:/root/.bitcoin --name bitcoin $imagename
